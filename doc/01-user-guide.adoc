...
;; You can check if you are dealing with a flatland ordered set
(oset/ordered-set? (oset/ordered-set :a :b :c))
=> true
----

In Clojure, you can use https://clojure.org/reference/transients[transient] on ordered sets:

[source,clojure]
----
(def t (transient (oset/ordered-set)))
(oset/ordered-set? t)
=> false
(def os (persistent! t))
(oset/ordered-set? os)
...
;; You can check if you are dealing with a flatland ordered map
(omap/ordered-map? (omap/ordered-map :a 1))
=> true
----

In Clojure, you can use https://clojure.org/reference/transients[transient] on ordered maps:

[source,clojure]
----
(def t (transient (omap/ordered-map)))
(omap/ordered-map? t)
=> false
(def om (persistent! t))
(omap/ordered-map? om)
