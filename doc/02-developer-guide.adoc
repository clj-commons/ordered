...
== Audience
You want contribute to, or learn about, the development of the `ordered` library.
...
* Please read the link:doc/CONTRIBUTING.adoc[Contributing Guidelines]
...
=== Linting
We use https://github.com/borkdude/clj-kondo[clj-kondo] and https://github.com/jonase/eastwood[eastwood] to lint `ordered` source code.
...
The `lint-kondo` task will build the clj-kondo cache when it is missing or stale.
If you want to force a rebuild of the cache run:
----
bb lint-kondo --rebuild
----

The `lint-docstrings` task checks that public API docstrings match between `.clj` and `.cljs` implementations.
Moving to `.cljc` would avoid this, but for now we want to avoid this refactor.

We don't run it on CI, but you can also optionally lint with https://github.com/noahtheduke/splint[splint]:
----
bb lint-splint --help
----
