...
;; this project started with no prefix, moved to "Release-", but we prefer "v" as a version tag prefix
(def legacy-version-tag-prefixes ["Release-" ""])
...
(comment
  ...
  ;; new format
  (parse-raw-tag "boo refs/tags/v1.15.13")
  ;; => {:tag "v1.15.13", :version "1.15.13"}

  ;; very old format
  (parse-raw-tag "boo refs/tags/1.5.6")
  ;; => {:tag "1.5.6", :version "1.5.6"}

  ;; recent format
  (parse-raw-tag "boo refs/tags/Release-1.15.12")
  ;; => {:tag "Release-1.15.12", :version "1.15.12"}
